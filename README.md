...
## Problema a resolver
Se debe procesar con periodicidad anual un conjunto de datos de vuelos de avión que comprende los años 2009 a 2018
...
la demora promedio y un indicador que diga si la demora experidmentada ese día fue anómala o no
- Un gráfico por cada aeropuerto donde se pueda visualizar la información anterior en una estructura
de archivos que permita identificar de manera sencilla el año y aeropuerto del que se trate.
...
y no tanto en el resto. Por lo tanto, no sería conveniente adoptar alguna solución que requiera mucho mantenimiento anual sino más bien algo que pueeda manejar las ráfagas de mayor uso de manera eficiente.   
...
como se ve en el digrama pero parcialmente usa ademas al menos una AZ adicional para ciertas cuestiones de mantenimiento debido a la manera en que funciona el servicio de RDS. Si fuera necesario un deploy multi AZ por 
...
Un aspecto positivo del uso de RDS con postgresql en lugar de otras opciones más especificas como Aurora es que 
existe menor dependencia del cloud provider en este aspecto, en caso de necesitar mudar de proveedor en algun momento.
...
datos en una estructura de carpetas por año. Si fuera necesario separar estos dos usos en dos buckets diferentes, eso se podría hacer de manera sencilla también. Este bucket tampoco tiene acceso público. 

**Bastion host:** Este host se encuentra en la subred pública y puede ser accedido desde internet. Se usa para acceder
o administrar el resto de los componentes. Esto permite que el acceso a los componentes desde el exterior esté limitado a un solo punto para disminuir posibles riesgos de seguridad.
...
- RDS: Crear el deploy y hacer un grupo de subredes que contenga a las subredes privadas. Colocar la base de datos en ese grupo de subredes. Deshabilitar el acceso público.
- Worker: Colocarlo en la subred privada que sea de la misma AZ donde se encuentra el RDS. Desde el Bastion host conectarse al Worker para configurar e instalar todo lo necesario como scripts de python, dockerfiles, shell scripts y archivos de configuración como el "config.ini".
...
- Quicksight: Para poder acceder a los datos del RDS se debe primero crear una conexion VPC y asignarle un security group que permita el acceso a los recursos necesarios

### Archivo config.ini
Archivo de configuracion donde se ingresan ciertos parámetros. Se encuentra en la carpeta "conf"
...
Configuracion de algoritmos de anomalías. Se encuentran implementados el algoritmo "Robusut Covariance" que parece funcionar de manera mas precisa para identificar las anomalías pero toma mucho mas tiempo de corrida que el "Isolation Forest". El factor de contaminación o porcentaje de outliers normalmente esperado en los datos fue seteado en 0.06 segun un análisis exploratorio de los datos. Ver seccion al respecto. El parámetro de random seed se setea en 42 por obvias razones.
...
Cada archivo de datos de entrada contiene alrededor de 6 millones de datos que luego de ser agregados por aeropuerto genera alrededor de 100.000 datos. Analizando luego esto datos agregados para el año 2009 se puede obtener lo siguiente:
...
Una vez determinado un valor para el parámetro "contamination" se puede usar para evaluar cuál algoritmo de detección es mas adecuado para los datos. Sería de esperar que la gran mayoría de los datos (inliers) estuvieran concentrados en valores cercanos a 0 y que los datos anómalos (outliers) estuvieran alejados del 0 y que esto se repitiera a los largo de todo el periodo de tiempo analizado. El resultado de varios algoritmos de detección de anomalías se puede ver en la siguiente figura:

![Alt text](outlier_algorithm.png?raw=true "Title")

Se puede ver que los algoritmos que mejor resutado arrojan son el "Robust Covariance" y el "Isolation Forest". El resto no parecen arrojar resultados adecuados. Además se puede ver que el "Isolation Forest" tarda significativamente menos tiempo en correr que el "Robust Covariance" aunque parece ser un poco menos preciso. A partir de esto se decido implementar los dos algoritmos más apropiados y la posiblidad de seleccionar cual usar mediante un archivo de configuración. 
